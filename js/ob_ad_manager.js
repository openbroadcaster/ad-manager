...
OBModules.ObAdManager.init_menu = function () {
    ...
    OB.UI.addSubMenuItem(
        ...
        "Ad Manager Settings",
        "obadmanager",
        OBModules.ObAdManager.load_settings,
        20,
        "ad_manager_settings",
    );
};
...
OBModules.ObAdManager.load = function () {
    OB.API.post("obadmanager", "get_settings", {}, function (settings) {
        ...
        if (!settings.data.enabled || !settings.data.disabled || !settings.data.timezone) {
            $("#ob_ad_manager_message")
                .obWidget(
                    "error",
                    'The ad manager does not seem to be configured properly.  Please adjust settings in "admin -> ad manager settings", or contact an administrator for assistance.',
                )
                .show();
            return;
        }
        ...
        $("#ob_ad_manager_drop").droppable({
            drop: function (event, ui) {
                if ($(ui.draggable).attr("data-mode") != "media") {
                    alert("Only media items are supported.");
                    return;
                }

                if ($(".sidebar_search_media_selected").length != 1) {
                    alert("You can manage only one media item at a time.");
                    return;
                }
...
OBModules.ObAdManager.get_items = function () {
    OB.API.post("obadmanager", "get_items", {}, function (response) {
        ...
        if (!$("#ob_ad_manager_expired tbody").length)
            $("#ob_ad_manager_expired").append('<tbody><tr><td colspan="4">No ads found.</td></tr></tbody>');
        if (!$("#ob_ad_manager_current tbody").length)
            $("#ob_ad_manager_current").append('<tbody><tr><td colspan="4">No ads found.</td></tr></tbody>');
        if (!$("#ob_ad_manager_upcoming tbody").length)
            $("#ob_ad_manager_upcoming").append('<tbody><tr><td colspan="4">No ads found.</td></tr></tbody>');
...
OBModules.ObAdManager.tbody_html = function (items) {
    ...
    $.each(items, function (index, item) {
        var $tr = $("<tr></tr>");
        $tr.append(
            '<td class="ob_ad_manager_media">' +
                htmlspecialchars(item.media_artist + " - " + item.media_title) +
                "</td>",
        );
...
OBModules.ObAdManager.delete_item = function (confirm) {
    if (confirm) {
        ...
        OB.UI.confirm(
            "Delete this item?",
            function () {
                OBModules.ObAdManager.delete_item(true);
            },
            "Yes, Delete",
            "No, Cancel",
            "delete",
    ...
    OB.API.post("obadmanager", "get_settings", {}, function (response) {
        OB.API.post("players", "search", {}, function (players_response) {
            ...
            for (var i in OB.Settings.categories) {
                $("#ob_ad_manager_enabled_category").append(
                    '<option value="' +
                        OB.Settings.categories[i].id +
                        '">' +
                        htmlspecialchars(OB.Settings.categories[i].name) +
                        "</option>",
                );
                $("#ob_ad_manager_disabled_category").append(
                    '<option value="' +
                        OB.Settings.categories[i].id +
                        '">' +
                        htmlspecialchars(OB.Settings.categories[i].name) +
            ...
            $("#ob_ad_manager_enabled_category").change(function () {
                OBModules.ObAdManager.settings_update_genre_list("enabled");
            });
            $("#ob_ad_manager_disabled_category").change(function () {
            ...
            if (response.data.enabled)
                $("#ob_ad_manager_enabled_category").val(response.data.enabled.media_category_id);
            if (response.data.disabled)
                $("#ob_ad_manager_disabled_category").val(response.data.disabled.media_category_id);
            ...
            $.each(players, function (index, player) {
                $("#ob_ad_manager_player_cache_list").append(
                    '<div><input type="checkbox" class="ob_ad_manager_clear_cache" value="' +
                        player.id +
                        '"> ' +
            ...
            if (response.data.players_clear_cache.length)
                $.each(response.data.players_clear_cache, function (index, player_id) {
...
OBModules.ObAdManager.settings_update_genre_list = function (field) {
    ...
    for (var i in OB.Settings.genres) {
        if (OB.Settings.genres[i].media_category_id == selected_category)
            $("#ob_ad_manager_" + field + "_genre").append(
                '<option value="' +
...
OBModules.ObAdManager.save_settings = function () {
    ...
    OB.API.post(
        "obadmanager",
        "save_settings",
        { enabled: enabled_id, disabled: disabled_id, timezone: timezone, players_clear_cache: players_clear_cache },
        function (response) {
